"""Aggregates New Mexico-located listings from every taildraggers.com
...
ones whose location ends in ", NM".
...
Jersey repos' approach, with the state filter swapped to "NM".
"""
...
PAGE_TITLE = "Taildragger Ads in New Mexico"
STATE_ABBR = "NM"
...
_NEW_MEXICO_LOCATION_RE = re.compile(r",\s*NM\s*$", re.IGNORECASE)


def _is_new_mexico(location: str) -> bool:
    return bool(_NEW_MEXICO_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        new_mexico_rows = [row for row in rows if _is_new_mexico(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(new_mexico_rows)} in New Mexico")
        all_listings.extend(new_mexico_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No New Mexico listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique New Mexico listings: {len(listings)}")
