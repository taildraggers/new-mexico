...
Daily aggregator of New Mexico-located taildragger classified
...
[New Jersey](https://github.com/taildraggers/new-jersey) repos' approach,
with the state filter swapped to "NM".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, NM` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in New Mexico"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/new-mexico/`
...
```html
<iframe
  src="https://taildraggers.github.io/new-mexico/"
  title="Taildragger Ads in New Mexico"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_NEW_MEXICO_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
